    ...
    <div class="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div class="space-y-6">
            <section class="surface-card">
                <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                    <div>
                        <p class="page-eyebrow">Thông tin chính</p>
                        <h2 class="mt-1 text-lg font-semibold text-slate-950">Sinh viên và hồ sơ</h2>
                    </div>
                ...
                <dl class="mt-5 grid gap-x-8 gap-y-5 sm:grid-cols-2">
                    <div>
                        <dt class="text-xs font-semibold tracking-wide text-slate-500 uppercase">Sinh viên</dt>
                        <dd class="mt-1.5 font-semibold text-slate-900">{{ $document->student?->full_name ?? $document->student_code }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-semibold tracking-wide text-slate-500 uppercase">Loại hồ sơ</dt>
                        <dd class="mt-1.5 text-slate-800">{{ $document->documentType?->name ?? 'Không xác định' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-semibold tracking-wide text-slate-500 uppercase">Ngày nộp</dt>
                        <dd class="mt-1.5 text-slate-800">{{ $document->submitted_at->format('d/m/Y H:i') }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-semibold tracking-wide text-slate-500 uppercase">Ngày hoàn tất</dt>
                        <dd class="mt-1.5 text-slate-800">{{ $document->completed_at?->format('d/m/Y H:i') ?? '—' }}</dd>
                    </div>
                    <div class="sm:col-span-2">
                        <dt class="text-xs font-semibold tracking-wide text-slate-500 uppercase">Lý do</dt>
                        <dd class="mt-1.5 whitespace-pre-line text-sm leading-6 text-slate-700">
                            @php
                            $reasonNote = filled($document->note)
                            ? $document->note
                            : $document->statusHistory
                            ->sortByDesc(fn ($entry) => $entry->changed_at?->timestamp ?? 0)
                            ->first(fn ($entry) => filled($entry->note))
                            ?->note;
                            @endphp
                            @if (filled($document->invalid_reason) || filled($reasonNote))
                            @if (filled($document->invalid_reason))
                            <p>{{ $document->invalid_reason }}</p>
                            @endif
                            @if (filled($reasonNote) && $reasonNote !== $document->invalid_reason)
                            <p @class(['mt-1.5'=> filled($document->invalid_reason)])>{{ $reasonNote }}</p>
                            @endif
                            @else
                            —
                            @endif
        ...
        <aside class="space-y-5">
            ...
            <section class="surface-card">
                ...
                <h2 class="mt-1 font-semibold text-slate-950">Chuyển trạng thái</h2>
                <form method="POST" action="{{ route('documents.status', $document) }}" class="mt-4 space-y-3" data-status-form>
                    ...
                    <div>
                        <label for="transition_note" class="mb-2 block text-sm font-semibold text-slate-700">Lý do</label>
                        <textarea id="transition_note" name="transition_note" rows="3" maxlength="500" class="form-control" placeholder="Nhập lý do (nếu có)">{{ old('transition_note') }}</textarea>
                    </div>
